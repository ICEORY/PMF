...
class Inference(object):
    def __init__(self, settings: Option, model: nn.Module, recorder: Recorder):
        ...
        self.val_loader, self.pv_loader = self._initDataloader()
    ...
    def _initDataloader(self):
        ...
        val_perspective_loader = pc_processor.dataset.PerspectiveViewLoaderV2(
            dataset=valset,
    ...
    def run(self):
        ...
        with torch.no_grad():
            ...
            for i, (proj_data, xy_index, depth, _, _) in enumerate(self.val_loader):
                ...
                if "PMF" in self.settings.net_type:
                    pred_output, _ = self.model(
                        pcd_feature, img_feature)
                else:
                    pred_output = self.model(pcd_feature)
                                    
                # do crop
                pred_output = pred_output[:, :, h_pad//2: h_pad//2+input_label.size(1), 
                ...
                pred_np_origin = self.pv_loader.dataset.class_map_lut_inv[pred_np].astype(np.uint32)

                if self.settings.has_label:
                    sem_label, _ = self.pv_loader.dataset.loadLabelByIndex(
                        i)
                    self.evaluator.addBatch(
                        pred_np, self.pv_loader.dataset.class_map_lut[sem_label])

                seq_id, frame_id = self.pv_loader.dataset.parsePathInfoByIndex(
                    i)
        ...
        for i, iou in enumerate(cls_iou.cpu()):
            if i not in [0]:
                cls_eval_table.add_row([i, self.pv_loader.dataset.mapped_cls_name[i], iou.item(), cls_acc[i].cpu(
        ...
        for i in range(self.settings.n_classes):
            distribution_table.add_row([self.pv_loader.dataset.mapped_cls_name[i], dist_data[i].item(), (dist_data[i]/dist_data.sum()).item()])
        ...
        for i in range(1, self.settings.n_classes):
            table_title.append("{}".format(
                self.pv_loader.dataset.mapped_cls_name[i]))
        ...
        for i, iou in enumerate(cls_piou.cpu()):
            if i not in [0]:
                cls_eval_table.add_row([i, self.pv_loader.dataset.mapped_cls_name[i], iou.item(), cls_pacc[i].cpu(
        ...
        dist_data = conf_matrix.sum(0)
        for i in range(self.settings.n_classes):
            distribution_table.add_row([self.pv_loader.dataset.mapped_cls_name[i], dist_data[i].item()])
        self.recorder.logger.info("---- Data Distribution -----")
        self.recorder.logger.info(distribution_table)
